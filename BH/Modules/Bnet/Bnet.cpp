...
void Bnet::OnGameExit() {
	...
	if (std::regex_search(Bnet::lastName, match, Bnet::reg) && match.size() == 3) {
		...
		// Handle incrementing the number
		count++;
		...
		// Generate new game name with incremented count
		char buffer[16];
		sprintf_s(buffer, sizeof(buffer), "%s%d", name.c_str(), count);
		lastName = std::string(buffer);
	}
	else {
		// If there's no number at the end, append "1" to the current game name
		if (lastName.length() < 15) {
			lastName.append("1");
		}
		else {
			// Handle edge case: if appending "1" would exceed the length limit
			lastName = lastName.substr(0, 14).append("1");
		}
	}

	// Re-install the patches for the next game
	InstallPatches();
}
